...
export const usersApi = {
  ...
  // data may include: { name, email, password, roleId, propertyIds: number[] }
  create: (data) => request('/users', { method: 'POST', body: data }),
  update: (userId, data) => request(`/users/${userId}`, { method: 'PATCH', body: data }),
  delete: (userId) => request(`/users/${userId}`, { method: 'DELETE' }),
  changePassword: (userId, data) => request(`/users/${userId}/change-password`, { method: 'POST', body: data }),
...
export const companyApi = {
  // Admin only: create a new company
  create: (data) => request('/companies', { method: 'POST', body: data }),
  // Get current user's company
  getMyCompany: () => request('/companies/my'),
...
// ===== SELF CHECK-IN API (public, no auth) =====
// Requires a token generated by the backend (JWT containing bookingId).
// Token is generated via POST /properties/bookings/{bookingId}/generate-checkin-token (needs backend impl).
// The self-checkin form accepts: { guests: [{ name, govIdType, govIdNumber, govDocumentUrl, isPrimary }] }
export const selfCheckinApi = {
  submit: (token, guestData) => publicRequest(
    `/public/self-checkin/submit?token=${encodeURIComponent(token)}`,
    { method: 'POST', body: { guests: guestData } }
  ),
  // Generate a checkin token for a booking (requires auth — admin side)
  generateToken: (bookingId) => request(`/properties/bookings/${bookingId}/generate-checkin-token`, { method: 'POST' }),
};
